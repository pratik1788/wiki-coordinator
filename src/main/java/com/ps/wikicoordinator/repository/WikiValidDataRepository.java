...
public interface WikiValidDataRepository extends ReactiveCassandraRepository<WikiValidDataEntity,String> {
    @Query("select * from WIKI_VALID_DATA where DATA_MILESTONE =?0 and HOUR = ?1 PER PARTITION LIMIT ?2 ")
    ...
    @Query("select count(1) from WIKI_VALID_DATA where DATA_MILESTONE =?0 and HOUR = ?1 ")
    Mono<Integer> getPageCountByDataMilestoneAndHour(int dataMilestone, int hour );

    @Query("select sum(NON_UNIQUE_VIEWS) from WIKI_VALID_DATA where DATA_MILESTONE =?0 and HOUR = ?1 ")
